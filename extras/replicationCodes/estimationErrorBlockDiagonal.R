...
#### Data generating scheme is block-diagonal sparsity ####
#### n = 500, 1000; p = 1000, 5000; 
#### k = 10; R = 50; pi0 = 0.85; 
#### save.dir is the directory under which result should be saved

provideEstimationErrorsCase2 <- function(n, 
                                         p, 
                                         k, 
                                         repIndex, 
                                         save.dir = NA) {
  ...
  length = floor((1 - sparseProp) * p)
  offset = ceiling((1 - (((1/(1 - sparseProp)) - 1)/(k-1))) * length)
  
  conditionVal <- p - (length + ((length - offset) * (k-1)))
  if(conditionVal < 0) {stop("Choose better length, offset!")}
  ...
  print(paste0("Lambda Sparsity = ", length(which(Lambda == 0)) / (p*k)))
  #myImagePlot(Lambda, F)   # plot heatmap of loading
