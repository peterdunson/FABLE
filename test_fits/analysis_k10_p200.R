...
# Match whatever filename the sim wrote. If you ran B=10 with the tagged
# name, point fname at that file instead.
fname <- file.path("test_fits", sprintf("sim_fable_%d_%d.rds", K, P))
# fname <- file.path("test_fits", sprintf("sim_fable_%d_%d_B10.rds", K, P))
...
p_cover <- ggplot(cover_long, aes(x = factor(param_set), y = coverage, fill = target)) +
  ...
  labs(x = "Parameter set", y = "Empirical coverage", fill = NULL,
       title = "FABLE coverage of Omega 95% CIs (K=10, P=200; dashed = 0.95)") +
  theme_minimal() +
...
p_mse <- results |>
  ...
  labs(x = "Parameter set", y = NULL,
       title = "FABLE recovery MSE (K=10, P=200, sparse blocks)") +
  theme_minimal()
print(p_mse)
